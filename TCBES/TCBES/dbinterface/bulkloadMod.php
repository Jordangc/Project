  ...
  for($i=0;$i<$num_rec;$i++)
  {
	...
	$query_location = 
	  ...
	  else
	  {
		...
		sprintf('insert into location_details (north_degree,west_degree,elevation_foot,waypoint,utm_zone,utm_band,utm_northing,utm_easting) 
			values (%f, %f, %f, %s, %s, %s, %f, %f);', $north_degree,$west_degree,$elevation,quote($waypoint),quote($zone),quote($band),$northing,$easting);
		$location_details_result = mysql_query($query_location_details, $connDBEHSP) or die(mysql_error($connDBEHSP));
			
        $query_location = 
        sprintf('insert into location 
             (location_detail_code, island_name,reserve_name,locality_name)
