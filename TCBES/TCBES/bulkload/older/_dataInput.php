...
$insert[] = "INSERT INTO upload_column_names(abbr3_collector, xml_name, attribute_name, name_type)VALUES (%s,%s,%s,%s)";
	...
	function insertData($file_name, $position_info){
		global $values, $DEBUG, $connDBEHSP;
		...
		if($DEBUG) echo '<br /> IN insertData ' ;
		if($DEBUG) print_r($position_info);
		foreach($position_info as $position){
			$position_reindexed[] = $position;
		}
		
		$fileStream = fopen($file_name, 'r');
		$line_no = 0;
		$line1 = fgets($fileStream);
		while($line = fgets($fileStream)){
			//echo "DEBUg in while";
			if($line == $line1){
				//echo "BLANK";
				echo "<br /> count " . count($position_reindexed);
				echo "<br /> lineno " . $line_no;
				if($line_no == count($position_reindexed)){
					echo "IN end of stuff";
					recursiveFail($position_reindexed[$line_no - 1]);
					unset($values);
				}
				$line_no = 0;
			}
			else{
				$values[$position_reindexed[$line_no]] = $line;
				$line_no++;
			}
		}
		fclose($fileStream);
	...
	function recursiveFail($column_to_insert){
		echo 'the column to insert' . $column_to_insert;
		global $insert, $columns, $values, $fk_down, $connDBEHSP, $PLACE_HOLDER, $DEBUG;
